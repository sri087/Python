...
class BillReseter():
    ...
    def __init__(self):
        ...
        self.auth = {'key': self.config['trello']['app_key'],
                     'token': self.config['trello']['token']
                    }
        self.params = {'dueComplete': 'false',  # reset the completed checkbox
                       'idList': self.config['trello']['to_pay']   # send the card back to 'To Pay' list
                      }
    ...
    def reset_bill(self, bill):
        ...
        for label in bill['labels']:
            ...
            if freq[:6] == 'Repeat':
                ...
                url = self.config['trello']['base_url'] + self.config['trello']['reset_card_url'] + bill['id']
                self.requestor.http_put(url,put_params)
    
    def get_paid_bills(self):
        url = self.config['trello']['base_url'] + self.config['trello']['paid_bills_url']
